...
Prefs.setValue = function (key, val) {
	if (!(key in Prefs._Defaults)) console.warn("unrecognized preference key '%s'", key);
	var date = new Date();
...
Prefs.getValue = function (key, fallback) {
	...
	for (var i = 0, len = lines.length; !val && i < len; ++i) {
		...
		if ((delim === key.length) && line.indexOf(key) === 0) val = line.substring(delim + 1);
	}
...
Prefs.getClutchPrefs = function (o) {
	if (!o) o = {};
	for (var key in Prefs._Defaults) o[key] = Prefs.getValue(key, Prefs._Defaults[key]);
	return o;
};
