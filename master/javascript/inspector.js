	...
	var data = {
		...
		needsExtraInfo = function (torrents) {
			...
			for (i = 0; tor = torrents[i]; i++)
				if (!tor.hasExtraInfo()) return true;
			return false;
		},
		refreshTorrents = function () {
			...
			if (ids && ids.length) {
				...
				if (needsExtraInfo(data.torrents)) $.merge(fields, Torrent.Fields.InfoExtra);
		...
		updateInspector = function () {
			...
			if (!torrents || !torrents.length) name = 'No Selection';
			else if (torrents.length === 1) name = torrents[0].getName();
			else name = '' + torrents.length + ' Transfers Selected';
			setTextContent(e.name_lb, name || na);
			// update the visible page
			if ($(e.info_page).is(':visible')) updateInfoPage();
			else if ($(e.peers_page).is(':visible')) updatePeersPage();
			else if ($(e.trackers_page).is(':visible')) updateTrackersPage();
			else if ($(e.files_page).is(':visible')) updateFilesPage();
		},
		/****
		 *****  GENERAL INFO PAGE
		 ****/
		updateInfoPage = function () {
			...
			if (torrents.length < 1) str = none;
				...
				for (i = 0; t = torrents[i]; ++i) {
					it = t.getStatus();
					if (it != baseline) isMixed = true;
					if (!t.isStopped()) allPaused = allFinished = false;
					if (!t.isFinished()) allFinished = false;
				}
				if (isMixed) str = mixed;
				else if (allFinished) str = 'Finished';
				else if (allPaused) str = 'Paused';
				else str = torrents[0].getStateString();
			}
			...
			if (torrents.length < 1) str = none;
				...
				for (i = 0; t = torrents[i]; ++i) {
					if (!t.needsMetaData()) {
						...
						v = t.getHaveValid();
						haveVerified += v;
						if (t.getPieceSize()) verifiedPieces += v / t.getPieceSize();
				...
				if (!haveUnverified && !leftUntilDone) str = fmt.size(haveVerified) + ' (100%)';
				else if (!haveUnverified) str = fmt.size(haveVerified) + ' of ' + fmt.size(sizeWhenDone) + ' (' + str + '%)';
				else str = fmt.size(haveVerified) + ' of ' + fmt.size(sizeWhenDone) + ' (' + str + '%), ' + fmt.size(haveUnverified) + ' Unverified';
			}
			...
			//
			if (torrents.length < 1) str = none;
			else if (sizeWhenDone == 0) str = none;
			else str = '' + fmt.percentString((100.0 * available) / sizeWhenDone) + '%';
			...
			if (torrents.length < 1) str = none;
				...
				if (f) str = fmt.size(d) + ' (' + fmt.size(f) + ' corrupt)';
				else str = fmt.size(d);
			}
			...
			if (torrents.length < 1) str = none;
				...
				if (torrents.length == 1) {
					...
					if (d == 0) d = torrents[0].getHaveValid();
				}
				else {
					for (i = 0; t = torrents[i]; ++i) {
						d += t.getDownloadedEver();
			...
			if (torrents.length < 1) str = none;
				...
				for (i = 0; t = torrents[i]; ++i) {
					if (baseline != t.getStartDate()) baseline = 0;
					if (!t.isStopped()) allPaused = false;
				}
				if (allPaused) str = stateString; // paused || finished
				else if (!baseline) str = mixed;
				else str = fmt.timeInterval(now / 1000 - baseline);
			}
			...
			if (!str.length) {
				if (baseline < 0) str = unknown;
				else str = fmt.timeInterval(baseline);
			}
			...
			if (torrents.length < 1) str = none;
				...
				if (d < 0) str = none;
				else if (d < 5) str = 'Active now';
				else str = fmt.timeInterval(d) + ' ago';
			}
			...
			if (torrents.length < 1) str = none;
				...
				for (i = 0; t = torrents[i]; ++i) {
					...
					size += t.getTotalSize();
					if (pieceSize != t.getPieceSize()) pieceSize = 0;
				}
				if (!size) str = none;
				else if (pieceSize > 0) str = fmt.size(size) + ' (' + pieces.toStringWithCommas() + ' pieces @ ' + fmt.mem(pieceSize) + ')';
				else str = fmt.size(size) + ' (' + pieces.toStringWithCommas() + ' pieces)';
			}
			...
			else setTextContent(e.comment_lb, str);
			//
			//  origin
			//
			if (torrents.length < 1) str = none;
				...
				for (i = 0; t = torrents[i]; ++i) {
					if (creator != t.getCreator()) mixed_creator = true;
					if (date != t.getDateCreated()) mixed_date = true;
				}
				if (mixed_creator && mixed_date) str = mixed;
				else if (mixed_date && creator.length) str = 'Created by ' + creator;
				else if (mixed_creator && date) str = 'Created on ' + (new Date(date * 1000)).toDateString();
				else str = 'Created by ' + creator + ' on ' + (new Date(date * 1000)).toDateString();
			}
		...
		addSubtreeToView = function (tor, parent, sub, i) {
			...
			if (sub.parent) addNodeToView(tor, div, sub, i++);
			if (sub.children)
				for (key in sub.children) i = addSubtreeToView(tor, div, sub.children[key]);
		...
		updatePeersPage = function () {
			...
			for (k = 0; tor = torrents[k]; ++k) {
				...
				html.push('<table class="peer_list">', '<tr class="inspector_peer_entry even">', '<th class="encryptedCol"></th>', '<th class="upCol">Up</th>', '<th class="downCol">Down</th>', '<th class="percentCol">%</th>', '<th class="statusCol">Status</th>', '<th class="addressCol">Address</th>', '<th class="clientCol">Client</th>', '</tr>');
				for (i = 0; peer = peers[i]; ++i) {
					parity = (i % 2) ? 'odd' : 'even';
					html.push('<tr class="inspector_peer_entry ', parity, '">', '<td>', (peer.isEncrypted ? '<div class="encrypted-peer-cell" title="Encrypted Connection">' : '<div class="unencrypted-peer-cell">'), '</div>', '</td>', '<td>', (peer.rateToPeer ? fmt.speedBps(peer.rateToPeer) : ''), '</td>', '<td>', (peer.rateToClient ? fmt.speedBps(peer.rateToClient) : ''), '</td>', '<td class="percentCol">', Math.floor(peer.progress * 100), '%', '</td>', '<td>', fmt.peerStatus(peer.flagStr), '</td>', '<td>', sanitizeText(peer.address), '</td>', '<td class="clientCol">', sanitizeText(peer.clientName), '</td>', '</tr>');
				}
				html.push('</table></div>');
			}
		...
		updateTrackersPage = function () {
			...
			for (i = 0; tor = torrents[i]; ++i) {
				html.push('<div class="inspector_group">');
				if (torrents.length > 1) html.push('<div class="inspector_torrent_label">', tor.getName(), '</div>');
				tier = -1;
				...
				for (j = 0; tracker = trackers[j]; ++j) {
					if (tier != tracker.tier) {
						if (tier !== -1) // close previous tier
							html.push('</ul></div>');
						tier = tracker.tier;
						html.push('<div class="inspector_group_label">', 'Tier ', tier + 1, '</div>', '<ul class="tier_list">');
					}
					...
					parity = (j % 2) ? 'odd' : 'even';
					html.push('<li class="inspector_tracker_entry ', parity, '"><div class="tracker_host" title="', sanitizeText(tracker.announce), '">', sanitizeText(tracker.host || tracker.announce), '</div>', '<div class="tracker_activity">', '<div>', lastAnnounceStatusHash['label'], ': ', lastAnnounceStatusHash['value'], '</div>', '<div>', announceState, '</div>', '<div>', lastScrapeStatusHash['label'], ': ', lastScrapeStatusHash['value'], '</div>', '</div><table class="tracker_stats">', '<tr><th>Seeders:</th><td>', (tracker.seederCount > -1 ? tracker.seederCount : na), '</td></tr>', '<tr><th>Leechers:</th><td>', (tracker.leecherCount > -1 ? tracker.leecherCount : na), '</td></tr>', '<tr><th>Downloads:</th><td>', (tracker.downloadCount > -1 ? tracker.downloadCount : na), '</td></tr>', '</table></li>');
				}
