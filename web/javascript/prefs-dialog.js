function PrefsDialog(remote) {
	var data = {
			...
			keys: ['alt-speed-down', 'alt-speed-time-begin', 'alt-speed-time-day', 'alt-speed-time-enabled', 'alt-speed-time-end', 'alt-speed-up', 'blocklist-enabled', 'blocklist-size', 'blocklist-url', 'dht-enabled', 'download-dir', 'encryption', 'idle-seeding-limit', 'idle-seeding-limit-enabled', 'lpd-enabled', 'peer-limit-global', 'peer-limit-per-torrent', 'peer-port', 'peer-port-random-on-start', 'pex-enabled', 'port-forwarding-enabled', 'rename-partial-files', 'seedRatioLimit', 'seedRatioLimited', 'speed-limit-down', 'speed-limit-down-enabled', 'speed-limit-up', 'speed-limit-up-enabled', 'start-added-torrents', 'utp-enabled'],
			// map of keys that are enabled only if a 'parent' key is enabled
			groups: {
				'alt-speed-time-enabled': ['alt-speed-time-begin', 'alt-speed-time-day', 'alt-speed-time-end'],
		...
		setGroupEnabled = function (parent_key, enabled) {
			...
			if (parent_key in data.groups) {
				...
				for (i = 0; key = keys[i]; ++i) root.find('#' + key).attr('disabled', !enabled);
			}
		},
		...
		getValue = function (e) {
			...
			case 'select-one':
				str = e.val();
				if (parseInt(str, 10).toString() === str) return parseInt(str, 10);
				if (parseFloat(str).toString() === str) return parseFloat(str);
				return str;
			default:
				return null;
			}
	...
	this.set = function (o) {
		...
		for (i = 0; key = keys[i]; ++i) {
			...
			case 'search':
				...
				if (e[0] !== document.activeElement) e.val(val);
				break;
			case 'select-one':
				e.val(val);
				break;
