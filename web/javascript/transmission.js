...
Transmission.prototype = {
	...
	initialize: function () {
		...
		$('#rename-button').click($.proxy(this.renameSelectedTorrents, this));
		$('#verify-button').click($.proxy(this.verifySelectedTorrents, this));
		...
		if (this.isMenuEnabled) this.createSettingsMenu();
		e = {};
	...
	loadImages: function () {
		for (var i = 0, row; row = arguments[i]; ++i) jQuery("<img>").attr("src", row);
	},
	/*
	...
	initializeSettings: function () {
		...
		if (this.isMenuEnabled) {
			...
			if (this[Prefs._SortDirection] === Prefs._SortDescending) $('#reverse_sort_order').selectMenuItem();
		}
	...
	createContextMenu: function () {
		...
		var bindings = {
			...
			context_rename: function () {
				tr.renameSelectedTorrents();
			},
			context_open: function () {
				tr.openSelectedTorrents();
			},
			...
			context_select_all: function () {
				tr.selectAll();
			},
			context_deselect_all: function () {
				tr.deselectAll();
			}
		};
		...
		$('ul#torrent_list').contextMenu('torrent_context_menu', {
			...
			onContextMenu: function (ev) {
				...
				if ((i !== -1) && !tr._rows[i].isSelected()) tr.setSelectedRow(tr._rows[i]);
				return true;
			}
		});
	},
	...
	onFreeSpaceResponse: function (dir, bytes) {
		...
		if (formdir == dir) {
			...
			if (bytes > 0) str = '  <i>(' + Transmission.fmt.size(bytes) + ' Free)</i>';
			else str = '';
			e.html('Capacity' + str + ':');
		}
	},
	...
	getAllTorrents: function () {
		var torrents = [];
		for (var key in this._torrents) torrents.push(this._torrents[key]);
		return torrents;
	},
	...
	scrollToRow: function (row) {
		...
		else if (innerHeight + scrollTop < offsetTop + offsetHeight) list.scrollTop(offsetTop + offsetHeight - innerHeight);
	},
	seedRatioLimit: function () {
		var p = this.sessionProperties;
		if (p && p.seedRatioLimited) return p.seedRatioLimit;
		return -1;
	},
	...
	indexOfLastTorrent: function () {
		for (var i = 0, r; r = this._rows[i]; ++i)
			if (r.getTorrentId() === this._last_torrent_clicked) return i;
		return -1;
	},
	...
	selectRange: function (row) {
		...
		else // select the range between the prevous & current
		{
			...
			for (var i = min; i <= max; ++i) this.selectRow(this._rows[i]);
		}
		this.callSelectionChangedSoon();
	},
	...
	keyDown: function (ev) {
		...
		if ((up || dn) && rows.length) {
			...
			if (anchor >= 0) {
				...
				if (ev.shiftKey) this.selectRange(r);
				else this.setSelectedRow(r);
			}
	...
	drop: function (ev) {
		...
		if (!ev.dataTransfer || !ev.dataTransfer.types) return true;
		for (i = 0; !uris && i < types.length; ++i)
			if (ev.dataTransfer.types.contains(types[i])) uris = ev.dataTransfer.getData(types[i]).split("\n");
	...
	updateFromTorrentGet: function (updates, removed_ids) {
		...
		for (i = 0; o = updates[i]; ++i) {
			...
			if ((t = this._torrents[id])) {
				...
				t.refresh(o);
				if (needed && !t.needsMetaData()) needinfo.push(id);
			}
			else {
	...
	uploadTorrentFile: function (confirmed) {
		...
		if (!confirmed) {
				...
				reader.onload = function (e) {
					...
					if (index > -1) {
						...
						remote.sendRequest(o, function (response) {
							if (response.result != 'success') alert('Error adding "' + file.name + '": ' + response.result);
						});
					}
				}
			...
			if (url != '') {
				...
				remote.sendRequest(o, function (response) {
					if (response.result != 'success') alert('Error adding "' + url + '": ' + response.result);
				});
			}
		}
	},
	...
	moveSelectedTorrents: function (confirmed) {
		var torrents = this.getSelectedTorrents();
		if (torrents.length) this.promptSetLocation(confirmed, torrents);
	},
	removeSelectedTorrents: function () {
		var torrents = this.getSelectedTorrents();
		if (torrents.length) this.promptToRemoveTorrents(torrents);
	},
	removeSelectedTorrentsAndData: function () {
		var torrents = this.getSelectedTorrents();
		if (torrents.length) this.promptToRemoveTorrentsAndData(torrents);
	},
	...
	renameSelectedTorrents: function () {
		var torrents = this.getSelectedTorrents();
		if (torrents.length != 1) dialog.alert("Renaming", "You can rename only one torrent at a time.", "Ok");
		else this.promptToRenameTorrent(torrents[0]);
	},
	...
	updateGuiFromSession: function (o) {
		...
		if (RPC._TurtleState in o) {
			...
			text = ['Click to ', (b ? 'disable' : 'enable'), ' Temporary Speed Limits (',
				...
				fmt.speed(o[RPC._TurtleDownSpeedLimit]), ' down)'
			].join('');
		...
		if (this.isMenuEnabled && (RPC._DownSpeedLimited in o) && (RPC._DownSpeedLimit in o)) {
			...
			if (!limited) e = menu.find('#unlimited_download_rate');
		...
		if (this.isMenuEnabled && (RPC._UpSpeedLimited in o) && (RPC._UpSpeedLimit in o)) {
			...
			e.html('Limit (' + fmt.speed(limit) + ')');
			if (!limited) e = menu.find('#unlimited_upload_rate');
			e.deselectMenuSiblings().selectMenuItem();
		}
	},
	...
	updateFilterSelect: function () {
		...
		names = [];
		for (name in trackers) names.push(name);
		names.sort();
		// build the new html
		if (!this.filterTracker) str = '<option value="all" selected="selected">All</option>';
		else str = '<option value="all">All</option>';
	...
	setInspectorVisible: function (visible) {
		if (visible) this.inspector.setTorrents(this.getSelectedTorrents());
	...
	sortRows: function (rows) {
		...
		for (i = 0; tor = torrents[i]; ++i) rows[i] = id2row[tor.getId()];
	},
	refilter: function (rebuildEverything) {
		...
		if (rebuildEverything) {
			$(list).empty();
			this._rows = [];
			for (id in this._torrents) this.dirtyTorrents[id] = true;
		}
		...
		for (i = 0; row = this._rows[i]; ++i) {
			if (row.getTorrentId() in this.dirtyTorrents) dirty_rows.push(row);
			else clean_rows.push(row);
		}
		// remove the dirty rows from the dom
		e = [];
		for (i = 0; row = dirty_rows[i]; ++i) e.push(row.getElement());
		$(e).detach();
		...
		for (i = 0; row = dirty_rows[i]; ++i) {
			id = row.getTorrentId();
			t = this._torrents[id];
			if (t && t.test(filter_mode, filter_text, filter_tracker)) tmp.push(row);
		...
		while (ci != cmax || di != dmax) {
			var push_clean;
			if (ci == cmax) push_clean = false;
			else if (di == dmax) push_clean = true;
			else {
				var c = Torrent.compareTorrents(clean_rows[ci].getTorrent(), dirty_rows[di].getTorrent(), sort_mode, sort_direction);
				push_clean = (c < 0);
			}
			if (push_clean) rows.push(clean_rows[ci++]);
			else {
				row = dirty_rows[di++];
				e = row.getElement();
				if (ci !== cmax) list.insertBefore(e, clean_rows[ci].getElement());
				else frag.appendChild(e);
				rows.push(row);
			}
		}
		...
		e = []
		for (i = 0; row = rows[i]; ++i) e.push(row.getElement());
		$(e).filter(":odd").addClass('even');
		$(e).filter(":even").removeClass('even');
		// sync gui
		this.updateStatusbar();
		if (old_sel_count !== $(list).children('.selected').length) this.selectionChanged();
	},
	...
	getDomainName: function (host) {
		...
		if (dot !== host.lastIndexOf('.')) host = host.slice(dot + 1);
		return host;
	},
	...
	getTrackers: function () {
		...
		for (var i = 0, torrent; torrent = torrents[i]; ++i) {
			...
			for (var j = 0, tracker; tracker = trackers[j]; ++j) {
				...
				if (announce in this.uriCache) uri = this.uriCache[announce];
				else {
				...
				if (!(uri.name in ret)) ret[uri.name] = {
					'uri': uri,
					'domain': uri.domain,
					'count': 0
				};
				if (names.indexOf(uri.name) === -1) names.push(uri.name);
			}
			for (var j = 0, name; name = names[j]; ++j) ret[name].count++;
		}
		return ret;
	},
	/***
